...
## Optional Claude Code Workflow Add-On

This section was requested as an extra handoff item so Codex can also support a Claude Code-first workflow in VS Code.

Important note:
- the primary recommended implementation is still to generate reports from `predict_today.py`
- however, if desired, the repo can also be set up so Claude Code can run the workflow with one reusable command / skill

---

## Even Better: Make Claude Code Do It With One Command

Claude Code supports reusable project instructions and workflow patterns.

A useful optional workflow would be a reusable command such as:

```text
/generate-mlb-predictions
```

The intended behavior of that command would be:

1. read today’s odds/model output data
2. create the correct dated markdown file
3. fill in all games
4. add the summary table at the end

This is best treated as a convenience layer on top of the Python pipeline, not a replacement for the model logic.

---

## Best Practical Approach

If adding Claude Code support, do these 3 things:

1. Create `CLAUDE.md` with project rules.
2. Save Odds API / model output into a dated JSON file in `/data`.
3. Use a daily Claude Code prompt or skill telling it exactly which file to read and exactly where to save the markdown.

This makes the workflow more reliable inside VS Code.
...
```text
E:/BettingModel/CLAUDE.md
```

Suggested content:

```md
# MLB Betting Project Instructions

You are working in an MLB betting model repository.

## Core rule
The Python model pipeline is the source of truth.
Do not invent picks, edges, or odds.
...
The frontend expects JSON output from `predict_today.py`.
Do not break that JSON structure.

## Report generation
When asked to generate MLB predictions reports:
- read the latest dated JSON/model output file in `mlb/data/`
  or run `predict_today.py` if instructed
- create a markdown report in:
  `E:/BettingModel/mlb/predictions/<Month> Predictions/`
- file names must be like:
  `April 14th 2026 Predictions.md`

## Betting rules
- bankroll is fixed at €500
- stake sizes allowed:
  - 0%
  - 1%
  - 2%
...
## Odds rules
- only use real odds from The Odds API or saved model output
- never fabricate fallback odds
- if odds are missing:
  `SKIP – Missing odds data`

## Decision rules
Every game must receive either:
- a bet recommendation
- or a skip

If edge is below threshold:
`SKIP – No clear edge`

## Markdown report requirements
Each report must include for every game:
- matchup
- pick
- odds
- stake %
...
At the end include a markdown table with:
`Game | Pick | Odds | Stake % | Stake € | Return € | Profit € | Decision`
```

---

## Suggested JSON Workflow for Claude Code

If Codex wants to support a Claude Code workflow, the cleanest setup is:
...
│   ├── data/
│   │   ├── predictions/
│   │   │   └── mlb-predictions-2026-04-14.json
│   ├── predictions/
│   │   └── April Predictions/
│   │       └── April 14th 2026 Predictions.md
│   └── scripts/
...
Recommended idea:
- `predict_today.py` continues generating the canonical prediction JSON
- that JSON can optionally be saved to disk
- Claude Code can then read that JSON and generate the markdown report
- this avoids hitting The Odds API unnecessarily from multiple places

---

## Suggested JSON Schema

If saving a dated JSON file for Claude Code to consume, it can look like this:
  ...
  "games": [
    {
      ...
      "homeL10RD": -6,
      "awayL10RD": 11,
      "homeSpName": "Pitcher A",
      "awaySpName": "Pitcher B",
      "homeSpEra": 4.28,
      "awaySpEra": 3.12,
      "homeSpWhip": 1.31,
      "awaySpWhip": 1.09
...
Notes:
- this is optional
- Codex can either save this structure directly from `predict_today.py`
- or keep the current stdout JSON and also add a dated file export

---

## Suggested Daily Claude Code Prompt

If a reusable skill/command is not added, this prompt can be used manually inside Claude Code:

```text
Read today’s MLB prediction JSON from the dated file in `E:/BettingModel/mlb/data/predictions/`.

Create a markdown predictions report for that date and save it under:
`E:/BettingModel/mlb/predictions/<Month> Predictions/`

Requirements:
- include every game
- if odds are missing, write: SKIP – Missing odds data
- if edge is below threshold, write: SKIP – No clear edge
- bankroll is €500
- only use 0% to 5% stake sizes
- calculate stake euro, return euro, and profit euro
- include concise edge reasons and one short risk note
- include the final markdown summary table
- do not invent odds or unsupported reasoning
```

---

## Optional Reusable Command / Skill Goal

If Codex implements a reusable Claude Code workflow, the target behavior is:
...
Expected result:
- read today’s model/odds output
- create the dated markdown report file
- include every game
- mark weak edges as skip
- append the summary table

Again, this should be treated as optional convenience automation around the Python source-of-truth workflow.

---

## Recommendation To Codex

If implementing both workflows, do it in this order:

1. Update `predict_today.py` to generate the markdown report directly.
2. Optionally add dated JSON export for local file consumption.
3. Optionally add `CLAUDE.md` and a reusable Claude Code command/skill workflow.

That keeps the betting logic inside Python and uses Claude Code only as an orchestration/helper layer.
